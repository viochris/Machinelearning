...
# null_data = pd.DataFrame(loan_data.isnull().mean() * 100)
...
null_data = pd.DataFrame(loan_data.isnull().sum() / loan_data.shape[0], columns = ['nullpct'])
...
tanggal = ['issue_d', 'earliest_cr_line', 'last_pymnt_d', 'last_credit_pull_d']
for tanggal in tanggal:
    date_columns(X_train, tanggal)
