...
print(house.isnull().sum()/ len(house))
print(house.isnull().sum()/ house.shape[0])
...
# del house['furnishingstatus']
house = pd.merge(house.reset_index(), status.reset_index())
house = house.drop(['furnishingstatus', 'index'], axis = 1)
print(house.head())


scaler = MinMaxScaler()
numvars = ['area', 'bedrooms', 'bathrooms', 'stories', 'parking','price']
house[numvars] = scaler.fit_transform(house[numvars])
print(house.head())
...
lm = LinearRegression()
scoring = 'r2'
score = cross_val_score(lm, x_train_rfe, y_train, cv = k_folds, n_jobs=1, scoring=scoring)
print(score)
print(round(score.mean(), 2))
lm.fit(x_train_rfe, y_train)


# VIF adalah cara untuk melihat seberapa masalahnya ketika kita punya beberapa data 
# yang sebenarnya berkaitan erat satu sama lain dalam model kita. Misalnya, jika kita ingin 
# memprediksi harga rumah dan kita menggunakan ukuran rumah dan jumlah kamar sebagai faktor-faktor, 
# ini masalahnya: mungkin ukuran rumah dan jumlah kamar sangat berkaitan, jadi model kita jadi kesulitan 
# untuk tahu mana yang lebih penting dalam memprediksi harga rumah.

# Jadi, VIF memberi tahu kita seberapa besar masalahnya. Jika VIF tinggi untuk satu faktor, 
# itu berarti faktor itu sangat berkaitan dengan yang lain. Dan ini membuat model kita jadi kesulitan. 
# Kami ingin VIF yang rendah, karena itu berarti faktor-faktor tersebut tidak terlalu berkaitan satu sama lain, 
# sehingga model kami lebih baik dalam memprediksi hasil yang benar. Jadi, semakin rendah VIF, semakin baik.
