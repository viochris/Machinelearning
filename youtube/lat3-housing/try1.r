...
print(colSums(is.na(house)) / dim(house)[1] * 100)
print(colSums(is.na(house)) / nrow(house) * 100)
print(colMeans(is.na(house)) * 100)
...
# house <- subset(house, select=- c(furnishingstatus, furnished))
# # house <- dplyr::select(house, -furnishingstatus, -furnished)
# # house <- dplyr::select(house, -c(furnishingstatus, furnished))
# print(head(house))
...
# TIDAK BISA KARENA MEMBUTUHKAN Y YANG SUDAH DIKETAHUI.
# set.seed(42)
...
# Menampilkan fitur yang dipilih dan tidak dipilih
...
# x_train_rfe <- cbind(Intercept = 1, x_train_rfe)
print(head(x_train_rfe))


set.seed(42)
ctrl <- trainControl(method="cv", number=5)
lm_model <- train(x = x_train_rfe, y = y_train, method="lm", trControl=ctrl, metric = "Rsquared")
print('accuracy mean')
print(lm_model$result)
print(lm_model$results$Rsquared)
print(mean(lm_model$results$Rsquared))
...
y_train_price <- predict(lm_model, x_train_rfe)
...
y_pred <- predict(lm_model, x_test_rfe)
print(head(y_pred))
